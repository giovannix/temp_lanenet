...
class CNNBaseModel(object):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME',
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            #print("filter_shape", filter_shape, type(filter_shape))
            w = tf.compat.v1.get_variable('W', filter_shape, initializer=w_init, use_resource=False)
            b = None

            if use_bias:
                b = tf.compat.v1.get_variable('b', [out_channel], initializer=b_init, use_resource=False)
    ...
    def depthwise_conv(input_tensor, kernel_size, name, depth_multiplier=1,
        ...
        with tf.compat.v1.variable_scope(name_or_scope=name):
            ...
            depthwise_filter = tf.compat.v1.get_variable(
                ...
                initializer=w_init, use_resource=False
            )
    ...
    def layernorm(inputdata, epsilon=1e-5, use_bias=True, use_scale=True,
        ...
        if use_bias:
            beta = tf.compat.v1.get_variable('beta', [channnel], initializer=tf.compat.v1.constant_initializer(), use_resource=False)
            beta = tf.reshape(beta, new_shape)
        else:
            beta = tf.zeros([1] * ndims, name='beta')
        if use_scale:
            gamma = tf.compat.v1.get_variable('gamma', [channnel], initializer=tf.compat.v1.constant_initializer(1.0), use_resource=False)
    ...
    def instancenorm(inputdata, epsilon=1e-5, data_format='NHWC', use_affine=True, name=None):
        ...
        beta = tf.compat.v1.get_variable('beta', [ch], initializer=tf.compat.v1.constant_initializer(), use_resource=False)
        beta = tf.reshape(beta, new_shape)
        gamma = tf.compat.v1.get_variable('gamma', [ch], initializer=tf.compat.v1.constant_initializer(1.0), use_resource=False)
        gamma = tf.reshape(gamma, new_shape)
        return tf.nn.batch_normalization(inputdata, mean, var, beta, gamma, epsilon, name=name)
    ...
    def dilation_conv(input_tensor, k_size, out_dims, rate, padding='SAME',
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            if use_bias:
                b = tf.compat.v1.get_variable('b', [out_dims], initializer=b_init, use_resource=False)
