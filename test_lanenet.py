...
def test_lanenet(image_path, weights_path):
    ...
    with sess.as_default():
        ...
        plt.title('lane_mask_image')
        plt.subplot(513)
        plt.imshow(binary_seg_image[0] * 255, cmap='gray')
        plt.title('binary_segmentation_image')
        plt.subplot(514)
        plt.imshow(mask_image[:, :, (2, 1, 0)])
        plt.title('instance_segmentation_image')
        plt.subplot(515)
        plt.imshow(embedding_image[:, :, (2, 1, 0)])
        plt.title('embedding_image')
        plt.savefig('final_plot.png', dpi=150, transparent=True, bbox_inches='tight')
